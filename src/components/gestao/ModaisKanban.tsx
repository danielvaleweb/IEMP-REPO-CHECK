...
interface ModalSaidaProps {
  ...
  onClose: () => void;
  onConfirm: (dados: { titulo: string; valor: number; operador_id: string; operador_nome: string; data_hora: string; anexo_url?: string }) => void;
}
          ...
          <div className="space-y-2">
            ...
            {tipoAnexo === "link" ? (
              <input
                type="url"
                placeholder="https://..."
                value={anexoUrl.startsWith("data:") ? "" : anexoUrl}
...
export const ListaSaidasModal: React.FC<{ isOpen: boolean; onClose: () => void; saidas: SaidaDespesa[] }> = ({ isOpen, onClose, saidas }) => {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-[1001] flex items-center justify-center pt-24 pb-6 px-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-xl max-h-[85vh] flex flex-col bg-[#120f17] border border-[#262036] text-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-[#262036] bg-[#181424]">
          <h3 className="text-lg font-bold">Listagem de Saídas / Despesas</h3>
          <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-white rounded-lg cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-3">
          {saidas.length === 0 ? (
            <p className="text-xs text-gray-400 text-center py-10">Nenhuma saída registrada.</p>
          ) : (
            saidas.map(s => (
              <div key={s.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-[#181424] border border-[#262036] gap-3">
                <div>
                  <strong className="text-sm font-bold block">{s.titulo}</strong>
                  <span className="text-xs text-gray-400 block mt-1">
                    Operador: {s.operador_nome || "Organizador"} • {s.data_hora}
                  </span>
                  {s.anexo_url && (
                    <a href={s.anexo_url} target="_blank" rel="noopener noreferrer" className="text-xs text-sky-400 hover:underline mt-2 flex items-center gap-1">
                      Visualizar Anexo
                    </a>
                  )}
                </div>
                <span className="text-base font-bold text-rose-400 shrink-0">
                  - R$ {s.valor.toFixed(2)}
                </span>
              </div>
            ))
...
export const ListaEntradasModal: React.FC<{ isOpen: boolean; onClose: () => void; entradas: CardMembro[] }> = ({ isOpen, onClose, entradas }) => {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-[1001] flex items-center justify-center pt-24 pb-6 px-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-xl max-h-[85vh] flex flex-col bg-[#120f17] border border-[#262036] text-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-[#262036] bg-[#181424]">
          <h3 className="text-lg font-bold">Listagem de Entradas / Arrecadado</h3>
          <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-white rounded-lg cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-3">
          {entradas.length === 0 ? (
            <p className="text-xs text-gray-400 text-center py-10">Nenhuma entrada registrada.</p>
          ) : (
            entradas.map(e => (
              <div key={e.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-[#181424] border border-[#262036] gap-3">
                <div>
                  <strong className="text-sm font-bold block uppercase">{e.membro_nome}</strong>
                  {e.comprovante_url && (
                    <a href={e.comprovante_url} target="_blank" rel="noopener noreferrer" className="text-xs text-sky-400 hover:underline mt-2 flex items-center gap-1">
